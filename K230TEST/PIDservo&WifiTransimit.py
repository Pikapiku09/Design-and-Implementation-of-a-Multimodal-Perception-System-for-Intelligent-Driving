...
class K230_Vehicle_Control:
    ...
    def H265_init(self):
        ...
        # 低延迟优化：设置GOP为1 (每帧都是I帧，减少编解码延迟)
        # 注意：会增加带宽，如需平衡可设为5-10
        if LOW_LATENCY_MODE:
            try:
                # 尝试设置低延迟参数
                self.encoder.SetChnAttr(VENC_CHN_ID_0, {
                    'gop': 1,  # GOP大小，1=全I帧(最低延迟)，增大可减少带宽
                    'statTime': 1,  # 统计时间
                    'srcFrmRate': TARGET_FPS,  # 源帧率
                    'dstFrmRate': TARGET_FPS,  # 目标帧率
                })
                print("低延迟模式已启用 (GOP=1)")
            except Exception as e:
                print(f"设置低延迟参数警告: {e}")
